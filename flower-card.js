...
class FlowerCard extends HTMLElement {
    ...
    this.innerHTML = `
      <ha-card>
        <style>
          ...
          .fc-bars { margin-top: 16px; text-align: left; }
          .fc-row {
            display: grid;
          ...
          .fc-label { opacity: 0.78; white-space: nowrap; }
          .fc-val {
            text-align: right;
            font-weight: 700;
            font-size: 12px;
          ...
          .fc-edit-btn:hover { background: rgba(255,255,255,0.28); }
          .fc-wrap-outer { position: relative; }
          ...
          .fc-setup-input:focus { border-color: rgba(124,252,0,0.7); }
          .fc-setup-select {
          ...
          .fc-setup-select:focus { border-color: rgba(124,252,0,0.7); }
          .fc-setup-select option { background: #1e1e1e; color: #fff; }
          .fc-search-box {
            background: rgba(255,255,255,0.12);
            border: 1px solid rgba(255,255,255,0.25);
            border-radius: 10px;
          ...
          .fc-search-box::placeholder { color: rgba(255,255,255,0.45); }
          .fc-field-wrap { display: flex; flex-direction: column; gap: 3px; }
          .fc-save-btn {
            width: 100%;
            margin-top: 14px;
            padding: 10px;
          ...
          .fc-save-btn:hover { opacity: 0.88; }
          .fc-cancel-btn {
            width: 100%;
            margin-top: 8px;
            padding: 8px;
          ...
          .fc-cancel-btn:hover { background: rgba(255,255,255,0.18); }
        </style>
  ...
  _renderSetup() {
    ...
    const sensorOptions = (selected) => `
      ...
      ${allSensors.map(id => `<option value="${id}" ${selected === id ? "selected" : ""}>${id}</option>`).join("")}
    `;
    ...
    this.querySelector("#fc-content").innerHTML = `
      <div class="fc-setup">
        ...
        ${cfg.soil ? `<button class="fc-cancel-btn" id="fc-cancel-btn">✕ Abbrechen</button>` : ""}
      </div>
    `;
    ...
    [
      ...
      searchEl.addEventListener("input", () => {
        ...
        Array.from(selectEl.options).forEach(opt => {
          opt.style.display = opt.value === "" || opt.value.toLowerCase().includes(term) ? "" : "none";
        });
      });
    });

    this.querySelector("#fc-save-btn").addEventListener("click", () => this._saveConfig());
  ...
  _update() {
    ...
    let icon = "🌸";
    if (soil === null)              icon = "❓";
    else if (soil < cfg.soil_warn_low)  icon = "🥀";
    else if (soil < 40)            icon = "🌼";
    else if (soil < 70)            icon = "🌸";
    else                           icon = "💧";

    // Status
    let status = "🟢 Alles OK";
    if (soil === null || ec === null)       status = "⚠️ Sensor nicht verfügbar";
    else if (soil < cfg.soil_warn_low)      status = "💧 Gießen empfohlen";
    else if (soil > cfg.soil_warn_high)     status = "🚨 Zu nass!";
    else if (ec < cfg.ec_warn_low)          status = "⚡ Düngen empfohlen";
    else if (ec > cfg.ec_warn_high)         status = "🚨 Zu viel Dünger!";
